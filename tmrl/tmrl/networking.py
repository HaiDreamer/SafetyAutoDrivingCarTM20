...
def ensure_run_dir(run_dir):
    run_dir = Path(run_dir) if run_dir is not None else Path("runs") / "default_run"
...
def append_jsonl(file_path, row):
    file_path = Path(file_path)
    with open(file_path, "a", encoding="utf-8") as f:
        f.write(json.dumps(row, ensure_ascii=False) + "\n")


def safe_info_value(info, key, default=None):
...
# BUFFER: stores rollout samples and a few episode statistics so different TMRL components can pass experience around.
...
def iterate_epochs(run_cls,
    ...
    checkpoint_jsonl = run_dir / "checkpoints.jsonl"
    ...
    try:
        ...
        while run_instance.epoch < run_instance.epochs:
            ...
            stats = run_instance.run_epoch(interface=interface)  # yield stats data frame (this makes this function a generator)
            epoch_log = {
                "timestamp": datetime.now().isoformat(timespec="seconds"),
                "epoch": int(getattr(run_instance, "epoch", -1)),
                "total_epochs": int(getattr(run_instance, "epochs", -1)),
                "epoch_runtime_sec": round(time.time() - epoch_start_time, 6),
                "total_runtime_sec": round(time.time() - train_start_time, 6),
            }
            append_jsonl(training_jsonl, epoch_log)
            if stats is not None:
                for idx, s in enumerate(stats):
                    try:
                        row = json.loads(s.to_json())
                    except Exception:
                        row = {"raw_stats_repr": str(s)}
                    row["timestamp"] = datetime.now().isoformat(timespec="seconds")
                    row["epoch"] = int(getattr(run_instance, "epoch", -1))
                    row["stats_index"] = idx
                    append_jsonl(training_jsonl, row)
            ...
            if run_instance.epoch % epochs_between_checkpoints == 0:
                ...
                # run_instance = load_run_instance_fn(checkpoint_path)
                append_jsonl(checkpoint_jsonl, {
                    "timestamp": datetime.now().isoformat(timespec="seconds"),
                    "epoch": int(getattr(run_instance, "epoch", -1)),
                    "checkpoint_path": str(checkpoint_path),
                    "save_runtime_sec": round(ckpt_runtime, 6),
                    "total_runtime_sec": round(time.time() - train_start_time, 6),
                })
...
def run_with_wandb(entity, project, run_id, interface, run_cls, checkpoint_path: str = None,
                   dump_run_instance_fn=None, load_run_instance_fn=None, updater_fn=None, run_dir=None):
    """
    Main training loop (remote).
...
def run(interface, run_cls, checkpoint_path: str = None,
        dump_run_instance_fn=None, load_run_instance_fn=None, updater_fn=None, run_dir=None):
    """
    Main training loop (remote).
    """
    dump_run_instance_fn = dump_run_instance_fn or dump_run_instance
    load_run_instance_fn = load_run_instance_fn or load_run_instance
    for stats in iterate_epochs(run_cls, interface, checkpoint_path, dump_run_instance_fn, load_run_instance_fn, 1, updater_fn, run_dir=run_dir):
        pass
...
class Trainer:
    ...
    def __init__(self,
        ...
        Args:
            ...
            header_size (int): number of bytes used by `tlspyo` headers
            ...
            updater_fn (callable): custom updater (`None` = no updater). 
            run_dir: directory where logs/artifacts are stored. If provided, this must be a function \
...
class RolloutWorker:
    ...
    def __init__(
        ...
        self.train_episode_log = self.run_dir / "train_episodes.csv"
        self.test_episode_log = self.run_dir / "test_episodes.csv"
        self.worker_events_log = self.run_dir / "worker_events.jsonl"
    ...
    def collect_train_episode(self, max_samples=None):
        ...
        # need to modify specific path
        log_file = "tmrl_episode_log.csv"
        ...
        drive_start_time  = time.time()     # start AFTER reset completes
        ...
        for i in iterator:
            obs, rew, terminated, truncated, info = self.step(
                ...
                last_step=i == max_samples - 1      # detect the final loop iteration then do something special ?
            )
            ret += rew
            steps += 1
            final_info = info
            # Extract progress from obs if available
            # In tmrl LIDAR mode, obs is a tuple: (speed, lidars, prev_actions...)
            # progress is NOT in obs by default either, so we track reward as proxy
            if terminated or truncated:
                break

        end_dt = datetime.now()
        runtime_sec = time.time() - start_time
        # Compute lap_time from runtime if episode finished 
        finished = bool(safe_info_value(final_info, "finished", False))
        lap_time = round(time.time() - drive_start_time, 3) if terminated else None
        
        finished = bool(terminated)  # terminated=True in tmrl means track completed
        ...
        row = {
            ...
            "episode_type": "train",
            "episode_index": self.train_episode_count,
            "seed": self.seed,
            "start_time": start_dt.isoformat(timespec="seconds"),
            "end_time": end_dt.isoformat(timespec="seconds"),
            "runtime_sec": round(runtime_sec, 6),
            "total_reward": round(float(ret), 6),
            "steps": steps,
            "terminated": terminated,
            "truncated": truncated,     # equal to "finished?"
            "lap_time": safe_info_value(final_info, "lap_time"),             # only set if car actually finished
            "finished": finished,
            # These below genuinely don't exist in tmrl info — remove or keep as None
            "progress": safe_info_value(final_info, "progress"),
            "crash_count": safe_info_value(final_info, "crash_count"),
            "off_track_count": None,
            "checkpoint_count": None,
        }

        append_csv_row(
            self.train_episode_log,
            fieldnames=list(row.keys()),
            row=row
        )

        append_jsonl(self.worker_events_log, {
            "timestamp": end_dt.isoformat(timespec="seconds"),
            "event": "train_episode_end",
            **row
        })
        ...
        print_with_timestamp(
            f"episode done | runtime_sec={runtime_sec:.2f} | total_reward={ret:.3f} | steps={steps}"
        )
        print_with_timestamp(f"DEBUG final_info keys: {list(final_info.keys())}")
    ...
    def run_episode(self, max_samples=None, train=False):
        ...
        if train:
            ...
            self.buffer.stat_train_steps = steps
            self.train_episode_count += 1
            log_path = self.train_episode_log
            episode_type = "train_evalmode"
            episode_index = self.train_episode_count
        else:
            self.buffer.stat_test_return = ret
            self.buffer.stat_test_steps = steps
            self.test_episode_count += 1
            log_path = self.test_episode_log
            episode_type = "test"
            episode_index = self.test_episode_count
        ...
        row = {
            ...
            "terminated": terminated,
            "truncated": truncated,
            "lap_time": safe_info_value(final_info, "lap_time"),
            "progress": safe_info_value(final_info, "progress"),
            "finished": safe_info_value(final_info, "finished"),
            "crash_count": safe_info_value(final_info, "crash_count"),
            "off_track_count": safe_info_value(final_info, "off_track_count"),
            "checkpoint_count": safe_info_value(final_info, "checkpoint_count"),
        }
        ...
        append_jsonl(self.worker_events_log, {
            "timestamp": end_dt.isoformat(timespec="seconds"),
            "event": "run_episode_end",
            **row
        })

        print_with_timestamp(
            f"{episode_type} episode done | runtime_sec={runtime_sec:.2f} | total_reward={ret:.3f} | steps={steps}"
        )
