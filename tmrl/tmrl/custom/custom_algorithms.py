...
import logging
from tmrl.custom.q_logger import log_metrics

DEBUG_MODE = False      # set True for verbose debug logging
...
def _get_csv_writer(filename: str, fieldnames: list):        
    """
    Open (or create) a CSV file and return (file_handle, csv.DictWriter).
    Writes the header row only when the file is newly created.
    Call file_handle.close() when done, or use in a context manager.
    """
    path = os.path.join(_CSV_LOG_DIR, filename)              
...
def _append_loss_row(writer, step: int, metrics: dict):      
    """Write one row to the CSV; missing keys become empty string."""
    row = {"step": step, "timestamp": time.strftime("%Y-%m-%d %H:%M:%S")}
    row.update(metrics)                                 
    writer.writerow(row)                                         
# ──────────────────────────────────────────────────────────────────────────────
...
class SpinupSacAgent(TrainingAgent):  # Adapted from Spinup
    ...
    def __post_init__(self):
        ...
        self._step_count = 0
        self._prev_logp_pi = None                                    # for approx KL tracking

    def __getstate__(self):                                          # ADDED: exclude file handles from pickle
        state = self.__dict__.copy()                        
        state.pop("_csv_file", None)                    
        state.pop("_csv_writer", None)                     
        return state                                           

    def __setstate__(self, state):                                   # ADDED: restore without file handles
        self.__dict__.update(state)                       
        # _csv_file/_csv_writer will be lazily re-opened in train() via hasattr guard 
    ...
    def train(self, batch):
        ...
        with torch.no_grad():
            ...
            if not cfg.DEBUG_MODE:
                ret_dict = dict(
                    ...
                    entropy=-logp_pi.detach().mean().item(),
                    ...
                    backup_min=backup.min().item(),
                    backup_max=backup.max().item(),
                )
            else:
        ...
        # ── Explained variance & approx KL ───────────────────────── ADDED
        with torch.no_grad():                          
            v_pred = ((q1 + q2) / 2).detach()                     
            returns = backup.detach()                              
            explained_var = (1 - (returns - v_pred).var() / (returns.var() + 1e-8)).item()       
            logp_now = logp_pi.detach()                          
            prev = getattr(self, "_prev_logp_pi", None)        # (checkpoint-resume safe)
            approx_kl = (prev - logp_now).mean().item() if prev is not None else 0.0 
            self._prev_logp_pi = logp_now                     
        ret_dict["explained_variance"] = explained_var           
        ret_dict["approx_kl"] = approx_kl                        
        # ─────────────────────────────────────────────────────────────

        # ── Write one row to CSV ──────────────────────────────────── ADDED
        if not hasattr(self, "_csv_file"):                           
            _sac_fields = ["step", "timestamp", "loss_actor",     
                           "loss_critic", "loss_entropy_coef",    
                           "entropy_coef", "entropy",              
                           "q1_mean", "q2_mean", "q1_min", "q2_min",
                           "q1_max", "q2_max", "q_gap",             
                           "backup_mean", "backup_min", "backup_max",
                           "explained_variance", "approx_kl"
                           "debug_log_pi", "debug_log_pi_std",
                           # debug 
                            "debug_logp_a2", "debug_logp_a2_std",
                            "debug_q_a1", "debug_q_a1_std",
                            "debug_q_a1_targ", "debug_q_a1_targ_std",
                            "debug_backup", "debug_backup_std",
                            "debug_q1", "debug_q1_std",
                            "debug_q2", "debug_q2_std",
                            "debug_diff_q1", "debug_diff_q1_std",
                            "debug_diff_q2", "debug_diff_q2_std",
                            "debug_diff_r_q1", "debug_diff_r_q1_std",
                            "debug_diff_r_q2", "debug_diff_r_q2_std",
                            "debug_diff_q1pt_qpt", "debug_diff_q2pt_qpt",
                            "debug_diff_q1_q1t_a2", "debug_diff_q2_q2t_a2",
                            "debug_diff_q1_q1t_pi", "debug_diff_q2_q2t_pi",
                            "debug_diff_q1_q1t_a", "debug_diff_q2_q2t_a",
                            "debug_diff_q1pt_qpt_std", "debug_diff_q2pt_qpt_std",
                            "debug_diff_q1_q1t_a2_std", "debug_diff_q2_q2t_a2_std",
                            "debug_diff_q1_q1t_pi_std", "debug_diff_q2_q2t_pi_std",
                            "debug_diff_q1_q1t_a_std", "debug_diff_q2_q2t_a_std",
                            "debug_r", "debug_r_std",
                            "debug_d", "debug_d_std",
                            "debug_a_0", "debug_a_0_std",
                            "debug_a_1", "debug_a_1_std",
                            "debug_a_2", "debug_a_2_std",
                            "debug_a1_0", "debug_a1_0_std",
                            "debug_a1_1", "debug_a1_1_std",
                            "debug_a1_2", "debug_a1_2_std",
                            "debug_a2_0", "debug_a2_0_std",
                            "debug_a2_1", "debug_a2_1_std",
                            "debug_a2_2", "debug_a2_2_std",
                        ]     
            self._csv_file, self._csv_writer = _get_csv_writer(    
                "sac_loss_log.csv", _sac_fields)                  
...
class REDQSACAgent(TrainingAgent):
    ...
    def train(self, batch):
        ...
        # ── Write one row to CSV ──────────────────────────────────── ADDED
        if not hasattr(self, "_csv_file"):                           # (checkpoint-resume guard)
            _redq_fields = ["step", "timestamp", "loss_actor",     
                            "loss_critic", "loss_entropy_coef",   
                            "entropy_coef"]                      
            self._csv_file, self._csv_writer = _get_csv_writer(   
                "redq_loss_log.csv", _redq_fields)              
        if not hasattr(self, "_step_count"):                
            self._step_count = 0                          
        self._step_count += 1                              
        if self._step_count % _CSV_LOG_INTERVAL == 0: 
