...
r'''
Receive telemetry from TrackMania/OpenPlanet over TCP
Convert a game screenshot into simple LIDAR-like distance readings
-> bridge between agent and game 
'''
    ...
    def __client_thread(self):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            while True:  # main loop
                # If multiple packets arrive at once, it discards older ones and keeps only the most recent full packet
...
def armin(tab):
    '''find the index of the first nonzero / first True element'''
    nz = np.nonzero(tab)[0]
    if len(nz) != 0:
        return nz[0].item()
    else:
...
class Lidar:
    ...
    def _set_axis_lidar(self, im):
        '''setup axis lidar, start point, threshold for endpoint color'''
        h, w, _ = im.shape
        self.h = h
        self.w = w
        ...
        list_ax_x = []
        list_ax_y = []
        for angle in range(90, 280, 10):        # scans from roughly left/front to right/front in 10-degree steps
            axis_x = []
            axis_y = []
            x = self.road_point[0]
            y = self.road_point[1]
