...
def detach(x):
    '''
    recursively removes tensors from the autograd graph.
    If it's a list/tuple, it recurses into each element
    '''
...
class PopArt(Module):
    ...
    In tmrl's SAC, the critic predicts Q-values (expected cumulative reward). Racing game rewards can vary wildly in scale 
    (e.g., small speed bonuses vs. large penalties). PopArt normalizes both the prediction and the target to keep them in a consistent 
    scale Zero, so the critic trains stably regardless of reward magnitude."""
    def __init__(self, output_layer, beta: float = 0.0003, zero_debias: bool = True, start_pop: int = 8):
        ...
        super().__init__()
        # start_pop: How many updates to wait before actually rescaling the output layer's weights. The first x = 8 updates just collect statistics without 
        #   touching the network. This prevents unstable rescaling at the very start of training when statistics are unreliable.
        self.start_pop = start_pop
        # beta: The EMA(a way to track a running average that weights recent data more than old data)
        #   smoothing rate for updating statistics. Small value = slow, stable tracking of target scale
        self.beta = beta
        # When True, beta is set to max(1/(updates+1), beta)
        self.zero_debias = zero_debias
        self.output_layers = output_layer if isinstance(output_layer, (tuple, list, torch.nn.ModuleList)) else (output_layer, ) #last layer, as tuple
        # shape and device for first layer bias
        ...
        # mean, mean square, standard deviation of Q-value target
        ...
        self.updates = 0    # Integer counter of how many times update() has been called

    @torch.no_grad()
    def update(self, targets):
        # Early training: 1/(updates+1) is large → fast adaptation → mean catches up to real data quickly
        # Later training: 1/(updates+1) becomes smaller than 0.0003 → switches to fixed beta → now it tracks slowly and stably, forgetting old data gradually
        #   => smaller beta over time => slower update, longer memory — the mean/std changes slowly and stably
