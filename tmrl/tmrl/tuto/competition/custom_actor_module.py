...
class SACTrainingAgent(TrainingAgent):
    ...
    def __init__(self,
        ...
        self._csv_step = 0
        self._prev_actor_loss = None
        # File sẽ nằm ngay tại thư mục bạn chạy lệnh `python custom_actor_module.py --trainer`
        self._csv_path = os.path.join(os.getcwd(), "loss_log.csv")
        with open(self._csv_path, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["step", "loss_critic", "loss_actor", "actor_delta", "critic_status"])
        print(f"[SACTrainingAgent] Logging losses to: {self._csv_path}")
    ...
    def train(self, batch):
        ...
        self._csv_step += 1
        lc = loss_q.detach().item()
        la = loss_pi.detach().item()

        # ── Phát hiện vấn đề ──────────────────────────────────────────
        critic_status = "ok"
        if math.isnan(lc) or math.isinf(lc):
            critic_status = "NaN/Inf"
            print(f"[STEP {self._csv_step}] CRITIC LOSS = {lc} — EXPLODED!")
        elif lc > 1e5:
            critic_status = "exploding"
            print(f"[STEP {self._csv_step}] CRITIC LOSS HIGH: {lc:.2f}")

        actor_delta = 0.0
        if self._prev_actor_loss is not None:
            actor_delta = abs(la - self._prev_actor_loss)
            if actor_delta > 50:
                print(f"[STEP {self._csv_step}] ⚠️  ACTOR LOSS SPIKE: Δ={actor_delta:.2f}  ({self._prev_actor_loss:.3f} → {la:.3f})")
        self._prev_actor_loss = la

        # ── Ghi CSV ───────────────────────────────────────────────────
        with open(self._csv_path, "a", newline="") as f:
            csv.writer(f).writerow([self._csv_step, lc, la, round(actor_delta, 4), critic_status])
